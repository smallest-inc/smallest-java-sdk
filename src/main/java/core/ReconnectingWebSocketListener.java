...
public abstract class ReconnectingWebSocketListener extends WebSocketListener {
   ...
   */
  public ReconnectingWebSocketListener(ReconnectingWebSocketListener.ReconnectOptions options,
  ...
  public void connect() {
    ...
    try {
      ...
      try {
        ...
        TimeoutException timeoutError = new TimeoutException("WebSocket connection timeout after " + 4000 + " milliseconds" + (retryCount.get() > 0 ? " (retry attempt #" + retryCount.get() : " (initial connection attempt)"));
        ...
        InterruptedException interruptError = new InterruptedException("WebSocket connection interrupted" + (retryCount.get() > 0 ? " during retry attempt #" + retryCount.get() : " during initial connection"));
        interruptError.initCause(e);
        onWebSocketFailure(null, interruptError, null);
      }
      catch (ExecutionException e) {
        Throwable cause = e.getCause() != null ? e.getCause() : e;
        String context = retryCount.get() > 0 ? "WebSocket connection failed during retry attempt #" + retryCount.get() : "WebSocket connection failed during initial attempt";
        RuntimeException wrappedException = new RuntimeException(context + ": " + cause.getClass().getSimpleName() + ": " + cause.getMessage());
  ...
  public void onFailure(WebSocket webSocket, Throwable t, Response response) {
    ...
    if (t != null) {
      ...
      enhancedError = new RuntimeException(errorContext + ": " + t.getClass().getSimpleName() + ": " + t.getMessage());
    ...
    public static final class Builder {
      ...
      public ReconnectOptions build() {
        ...
        if (minReconnectionDelayMs > maxReconnectionDelayMs) {
          throw new IllegalArgumentException("minReconnectionDelayMs (" + minReconnectionDelayMs + ") must not exceed maxReconnectionDelayMs (" + maxReconnectionDelayMs + ")");
        }
