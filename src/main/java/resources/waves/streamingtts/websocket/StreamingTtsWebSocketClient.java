...
package ai.smallest.resources.waves.streamingtts.websocket;
...
import ai.smallest.resources.waves.streamingtts.types.StreamTtsRequestMessage;
import ai.smallest.resources.waves.streamingtts.types.StreamTtsResponseMessage;
...
/**
 * WebSocket client for the streamingTts channel.
 * Provides real-time bidirectional communication with strongly-typed messages.
 */
public class StreamingTtsWebSocketClient implements AutoCloseable {
  ...
  private volatile Consumer<StreamTtsResponseMessage> receiveSynthesizeStreamingHandler;

  /**
   * Creates a new async WebSocket client for the streamingTts channel.
   */
  public StreamingTtsWebSocketClient(ClientOptions clientOptions) {
  ...
  public CompletableFuture<Void> connect() {
    ...
    String fullPath = "/waves/v1/streaming-tts/stream";
  ...
  /**
   * Sends a streamTtsRequest.message message to the server asynchronously.
   * @param message the message to send
   * @return a CompletableFuture that completes when the message is sent
   */
  public CompletableFuture<Void> synthesizeStreaming(StreamTtsRequestMessage message) {
    return sendMessage(message);
  }

  /**
   * Registers a handler for streamTtsResponse.message messages from the server.
   * @param handler the handler to invoke when a message is received
   */
  public void receiveSynthesizeStreaming(Consumer<StreamTtsResponseMessage> handler) {
    this.receiveSynthesizeStreamingHandler = handler;
  }
  ...
  private void handleIncomingMessage(String json) {
    try {
      ...
      StreamTtsResponseMessage receiveSynthesizeStreamingHandlerEvent = null;
      try {
        receiveSynthesizeStreamingHandlerEvent = objectMapper.treeToValue(node, StreamTtsResponseMessage.class);
      } catch (Exception e) {
      }
      if (receiveSynthesizeStreamingHandlerEvent != null) {
        if (receiveSynthesizeStreamingHandler != null) {
          receiveSynthesizeStreamingHandler.accept(receiveSynthesizeStreamingHandlerEvent);
        }
        return;
      }
      if (onErrorHandler != null) {
