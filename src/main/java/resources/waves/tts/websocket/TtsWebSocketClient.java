...
package ai.smallest.resources.waves.tts.websocket;
...
import ai.smallest.resources.waves.tts.types.TtsRequestMessage;
import ai.smallest.resources.waves.tts.types.TtsResponseMessage;
...
/**
 * WebSocket client for the tts channel.
 * Provides real-time bidirectional communication with strongly-typed messages.
 */
public class TtsWebSocketClient implements AutoCloseable {
  ...
  private volatile Consumer<TtsResponseMessage> receiveSynthesizeStreamingTtsHandler;

  /**
   * Creates a new async WebSocket client for the tts channel.
   */
  public TtsWebSocketClient(ClientOptions clientOptions) {
  ...
  public CompletableFuture<Void> connect() {
    ...
    String fullPath = "/waves/v1/tts/live";
  ...
  /**
   * Sends a ttsRequest.message message to the server asynchronously.
   * @param message the message to send
   * @return a CompletableFuture that completes when the message is sent
   */
  public CompletableFuture<Void> synthesizeStreamingTts(TtsRequestMessage message) {
    return sendMessage(message);
  }

  /**
   * Registers a handler for ttsResponse.message messages from the server.
   * @param handler the handler to invoke when a message is received
   */
  public void receiveSynthesizeStreamingTts(Consumer<TtsResponseMessage> handler) {
    this.receiveSynthesizeStreamingTtsHandler = handler;
  }
  ...
  private void handleIncomingMessage(String json) {
    try {
      ...
      TtsResponseMessage receiveSynthesizeStreamingTtsHandlerEvent = null;
      try {
        receiveSynthesizeStreamingTtsHandlerEvent = objectMapper.treeToValue(node, TtsResponseMessage.class);
      } catch (Exception e) {
      }
      if (receiveSynthesizeStreamingTtsHandlerEvent != null) {
        if (receiveSynthesizeStreamingTtsHandler != null) {
          receiveSynthesizeStreamingTtsHandler.accept(receiveSynthesizeStreamingTtsHandlerEvent);
        }
        return;
      }
      if (onErrorHandler != null) {
