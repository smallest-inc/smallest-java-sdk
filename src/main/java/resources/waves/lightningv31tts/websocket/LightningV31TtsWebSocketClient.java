...
package ai.smallest.resources.waves.lightningv31tts.websocket;
...
import ai.smallest.resources.waves.lightningv31tts.types.LightningV31TtsRequestMessage;
import ai.smallest.resources.waves.lightningv31tts.types.LightningV31TtsResponseMessage;
...
/**
 * WebSocket client for the lightningV31Tts channel.
 * Provides real-time bidirectional communication with strongly-typed messages.
 */
public class LightningV31TtsWebSocketClient implements AutoCloseable {
  ...
  private volatile Consumer<LightningV31TtsResponseMessage> receiveSynthesizeStreamingLightningV31Handler;

  /**
   * Creates a new async WebSocket client for the lightningV31Tts channel.
   */
  public LightningV31TtsWebSocketClient(ClientOptions clientOptions) {
  ...
  public CompletableFuture<Void> connect() {
    ...
    String fullPath = "/waves/v1/lightning-v3.1/get_speech/stream";
  ...
  /**
   * Sends a lightningV31TtsRequest.message message to the server asynchronously.
   * @param message the message to send
   * @return a CompletableFuture that completes when the message is sent
   */
  public CompletableFuture<Void> synthesizeStreamingLightningV31(
      LightningV31TtsRequestMessage message) {
    return sendMessage(message);
  }

  /**
   * Registers a handler for lightningV31TtsResponse.message messages from the server.
   * @param handler the handler to invoke when a message is received
   */
  public void receiveSynthesizeStreamingLightningV31(
      Consumer<LightningV31TtsResponseMessage> handler) {
    this.receiveSynthesizeStreamingLightningV31Handler = handler;
  }
  ...
  private void handleIncomingMessage(String json) {
    try {
      ...
      LightningV31TtsResponseMessage receiveSynthesizeStreamingLightningV31HandlerEvent = null;
      try {
        receiveSynthesizeStreamingLightningV31HandlerEvent = objectMapper.treeToValue(node, LightningV31TtsResponseMessage.class);
      } catch (Exception e) {
      }
      if (receiveSynthesizeStreamingLightningV31HandlerEvent != null) {
        if (receiveSynthesizeStreamingLightningV31Handler != null) {
          receiveSynthesizeStreamingLightningV31Handler.accept(receiveSynthesizeStreamingLightningV31HandlerEvent);
        }
        return;
      }
      if (onErrorHandler != null) {
